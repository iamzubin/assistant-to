...
func (db *DB) RecordEvent(agentID, eventType, details string) error {
	query := `
		INSERT INTO events (agent_id, event_type, details)
		VALUES (?, ?, ?)
	`
	_, err := db.Exec(query, agentID, eventType, details)
	if err != nil {
...
func (db *DB) GetAgentHistory(agentID string) ([]Event, error) {
	query := `
	...
	`
	return queryList(db, query, func(rows *sql.Rows) (Event, error) {
		var e Event
...
func (db *DB) GetLastHeartbeat(agentID string) (time.Time, error) {
	query := `
	...
	err := db.QueryRow(query, agentID).Scan(&lastSeen)
	if err != nil {
