...
- `mail_check`: recipient="opencodetest-{{.TaskID}}"
...
**Debug:** `buffer_capture` (agent_id="opencodetest-{{.TaskID}}", lines)

**Logging:** `log_event` (agent_id="opencodetest-{{.TaskID}}", type, details)

## Autonomy Protocol
1. **Start**: Log "Starting opencode test task {{.TaskID}}", mail plan
