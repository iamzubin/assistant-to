...
- `mail_check`: recipient="builder-2"
...
**Debug:** `buffer_capture` (agent_id="builder-2", lines)

**Logging:** `log_event` (agent_id="builder-2", type, details)

## Autonomy Protocol
1. **Start**: Log "Starting task 2", mail plan
...
**Title:** Rename project to dwight

**Description:**
Rename project from "dwight" to "dwight" everywhere - find and replace in all files including package.json, go.mod, imports, variable names, documentation, etc.

**Target Files:**
package.json, go.mod, *.go, *.ts, *.md, Makefile, docker-compose.yml, .env*, README*, *