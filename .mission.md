...
- `mail_check`: recipient="builder-7"
...
**Debug:** `buffer_capture` (agent_id="builder-7", lines)

**Logging:** `log_event` (agent_id="builder-7", type, details)

## Autonomy Protocol
1. **Start**: Log "Starting task 7", mail plan
...
**Title:** Fix token usage reporting

**Description:**
Token usage reporting is broken. The metrics system (internal/metrics/tokens.go) should parse token counts and costs from agent transcripts, store them in the database, and display them in the dashboard. Investigate and fix the token tracking pipeline.

**Target Files:**
internal/metrics/tokens.go, internal/metrics/tokens_test.go, internal/tui/dash.go